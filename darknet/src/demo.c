...
void *detect_in_thread(void *ptr)
{
  int frame_index = *(int *)ptr;
    ...
    dump_detections(display, demo_detections, demo_thresh, boxes, probs, 0, demo_names, demo_alphabet, demo_classes, frame_index);
...
void *display_in_thread(void *ptr)
{
    ...
    write_image_cv(buff[(buff_index + 1)%3], out, ipl);
    int c = cvWaitKey(1);
    if (c != -1) c = c%256;
    if (c == 27) {
        demo_done = 1;
...
void demo(char *cfgfile, char *weightfile, float thresh, int cam_index, const char *filename, char **names, int classes, int delay, char *prefix, int avg_frames, float hier, int w, int h, int frames, int fullscreen)
{
    ...
    if(!prefix){
      ...
      int num = (int)cvGetCaptureProperty(cap, CV_CAP_PROP_FRAME_COUNT);
      printf("count: %d, fps: %f, width: %d, height: %d\n", num, fps, width, height);
      out = cvCreateVideoWriter("out.avi", CV_FOURCC_DEFAULT,
    ...
    while(!demo_done){
        ...
        if(pthread_create(&detect_thread, 0, detect_in_thread, &count)) error("Thread creation failed");
        if(!prefix){
